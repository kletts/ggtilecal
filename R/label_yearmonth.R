...
#  for formatting date values, in the syntax of `strptime()` and passed to `tsibble::yearmonth()`
#'  
#'  - "%b" is "Jan", "Feb"
...
label_yearmonth <- function(formatstring="%b") { 
  ggplot2::as_labeller(\(x) format(tsibble::yearmonth(x), format = formatstring)) 
}